...
# B&uuml;hlmann--Gisler Estimators {#sec-appendix-e}

The functions `HBG()`, `sig.sq()`, and `tau()` defined below 
implement the estimation procedures provided in Chapter 8 of
@buhlmannCourseCredibilityTheory2005. 
These functions are used to implement 
@thm-hachemeister-formula.

Function `HBG()` (Hachemeister B&uuml;hlmann--Gisler) is the
main function used to calculate credibility estimates.
The function `sig.sq()` codes the formula for the estimator of
$\hat{\sigma}$ (@eq-credibility-estimator-sigma-squared),
and the function `tau()` codes the estimator for $\hat{\tau}$
that combines the variance estimators for the intercept
(@eq-estimator-variance-intercept)
and for the slope
(@eq-estimator-variance-slope) into a diagonal matrix.
